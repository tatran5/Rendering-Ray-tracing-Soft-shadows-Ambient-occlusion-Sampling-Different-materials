...
void Sampler::generateSamples(int& numSamples, SampleMode sampleMode, WarpMethod warpMethod)
{
    ...
    for(int i = 0; i < numSamples; ++i)
    {
        ...
        case GRID:
            // You want the center of the grid, so you want to add the
            sample = 1.f / sqrtVal * (glm::vec2((float) x, (float) y) + glm::vec2(0.5f)) ;
            break;

        case STRATIFIED:
            sample = glm::vec2((float) x / sqrtVal, (float) y / sqrtVal);
            sample = glm::vec2(sample.x + rng.nextFloat() / sqrtVal, sample.y + rng.nextFloat() / sqrtVal);
            break;
        }
